...
def run_training_loop(params):
    ...
    params['ep_len'] = params['ep_len'] or env.spec.max_episode_steps   #1000
    ...
    actor = MLPPolicySL(
        ...
        params['n_layers'],       # 2
        params['size'],          # 64
        learning_rate=params['learning_rate'], # 5e-3
    )

    # replay buffer
    replay_buffer = ReplayBuffer(params['max_replay_buffer_size']) # used to store past experiences that the agent can use for training.
                                                                   # Each experience typically includes the previous state, the action taken, the reward received, the next state, and whether the next state is terminal. The agent can then sample a batch of experiences from the replay buffer to update its policy or value function.
    #######################
    ## LOAD EXPERT POLICY
    #######################
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            # BC (Behaviral Cloning) training from expert data.
        ...
        training_logs = []
        for _ in range(1):#(params['num_agent_train_steps_per_iter']):
